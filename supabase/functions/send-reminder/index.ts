...
console.log(`Function "send-reminder" up and running!`);
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Attempting to send interview reminder to: ${registration.email} via SMTP.`);
    ...
    await send({
      ...
      subject: `Reminder: Your Interview with Career Redefine`,
      content: `Hi ${registration.full_name}, This is a friendly reminder for your interview with Career Redefine.`,
      html: `
        <h1>Interview Reminder</h1>
        <p>Hi ${registration.full_name},</p>
        <p>This is a friendly reminder for your interview with Career Redefine scheduled for:</p>
    ...
    return new Response(JSON.stringify({ message: "Reminder email sent successfully.", simulated: false }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      status: 200,
    })
  } catch (error) {
